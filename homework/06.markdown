...
* using mongoose, __create two schemas__:
	...
	* `ArticleSchema`
		...
		* <span class="warning">if you use embedded documents, you can skip using mongoose-url-slugs, and create slugs on your own</span>
			* [it seems like sub docs aren't supported](https://github.com/talha-asad/mongoose-url-slugs/issues/13)
			* (mainly because the plugin only works when save is explicitly called)
			* soooo.... instead, manually create a slug field, and set it to the lowercase version of the title, replacing spaces with dashes
